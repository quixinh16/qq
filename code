...
/*
    PapaFlokiRise
    https://www.PapaFlokiRise.finance
...
contract PapaFlokiRise is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public ERC20("PapaFlokiRise", "PapaFlokiRise") {
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "PapaFlokiRise: cannot send until trading is enabled");
        }
        if (amount == 0) {
        ...
        if (isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "PapaFlokiRise: fixed-sale has not started yet");
            ...
            if (!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "PapaFlokiRise: fixed-sale only available to certain participants at start");
            }
...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("PapaFlokiRise_Dividend_Tracker", "PapaFlokiRise_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function withdrawDividend() public override {
        require(false, "DividendTracker: withdrawDividend disabled. Use the 'claim' function on the main PapaFlokiRise contract.");
    }
